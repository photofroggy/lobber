...
Server.prototype.config_servers = function( servers ) {
    ...
    if( servers.hasOwnProperty( 'websocket' ) ) {
        ...
        try {
            ...
            server.listen( wsport, function() {
                console.log((new Date()) + '] lobber >> WebSocket server listening on port', wsport);
            } );
    ...
    if( servers.hasOwnProperty( 'socket' ) ) {
        ...
        try {
            ...
            var net = require('net');
            sport = servers.socket;
            
            var server = net.createServer(  );
            
            server.listen( sport, function() {
                console.log((new Date()) + '] lobber >> Socket server listening on port', sport);
            } );
